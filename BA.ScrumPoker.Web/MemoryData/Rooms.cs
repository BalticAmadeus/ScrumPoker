...
namespace BA.ScrumPoker.MemoryData
{
	public class Rooms
	{
		...
		private readonly Dictionary<string, RoomModel> _rooms;
		private readonly Dictionary<int, ClientModel> _clients;
		...
		public RoomModel CreateRoom()
		{
			lock (_sync)
			{
				for (var i = 10; i >= 0; i--)
				{
					var roomId = Base36Generator.GenerateString(6, _rng);
		...
		public bool Kick(string roomId, int clientId)
		{
			lock (_sync)
			{
				ClientModel client;
